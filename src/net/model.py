...
class FishSensorEstimator(nn.Module):
    def __init__(self, input_size=3, gru_hidden=64, mlp_hidden=128, h_out=H_OUT):
        ...
        # predice [sensor_diff, sensor_mean, v_flow] per ogni istante passato
        self.head_history = nn.Linear(gru_hidden, 3)

        # Testa futuro: predice [sensor_diff, sensor_mean, v_flow] al timestep t+1
    ...
    def forward(self, seq):
        ...
        returns:
            pred_history  (batch, h_out, 3)  =>  sensori agli ultimi h_out istanti
        ...
        # testa storia: ultimi h_out hidden state -> sensori passati
        pred_history = self.head_history(all_h[:, -self.h_out:, :])  # (batch, h_out, 3)
