...
class FishDataset(Dataset):
    ...
    def _process_episode(self, df: pd.DataFrame, window: int):
        ...
        # --- v_flow: usa la corrente del Dynamixel come proxy finché non c'è il sensore di flusso ---
        # v_flow = np.zeros(len(df), dtype=np.float32)           # placeholder zero
        # v_flow = df["v_flow"].values.astype(np.float32)        # decommentare quando disponibile
        v_flow = df["present_current_ma"].values.astype(np.float32)  # proxy: corrente motore
        ...
        self.norm_stats = {
            ...
            "vf_mean":  float(vf_mean),  "vf_std":  float(vf_std),  # proxy: corrente motore
        }
