...
class FishInverseEstimator(nn.Module):
    def __init__(self, input_size=3, gru_hidden=64, mlp_hidden=128, h_out=H_OUT):
        ...
        # input:  (batch, h_out, 3)        => storia dei valori sensoriali
        ...
        # (batch, h_out, gru_hidden) -> (batch, h_out, 1)
    ...
    def forward(self, seq):
        """
        seq:     (batch, h_out, 3)   => storia sensoriale normalizzata
                                        [sensor_diff, sensor_mean, current]

        returns:
            pred_history  (batch, h_out, 1)   => comandi agli ultimi h_out istanti passati
        ...
        # testa storia: tutti gli h_out hidden state -> comandi passati
        pred_history = self.head_history(all_h)          # (batch, h_out, 1)
