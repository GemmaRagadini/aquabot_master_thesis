...
def top_phase1_archs(storage, k=2):
    ...
    p1 = optuna.load_study(study_name="fish_inverse_phase1", storage=storage)
...
def best_phase2(storage):
    """
    La fase 2 crea studi fish_inverse_phase2_arch1/arch2: qui si sceglie
    ...
    for rank, arch in enumerate(archs, start=1):
        try:
            s = optuna.load_study(
                study_name=f"fish_inverse_phase2_arch{rank}", storage=storage
            )
        except Exception:
            continue
        ft = finite_trials(s)
...
if __name__ == "__main__":
    ...
    # ------------------------------------- Fase 2 - TPE per le top-2 arch
        ...
        for rank, best_arch in enumerate(archs, start=1):
            sub_study_name = f"fish_inverse_phase2_arch{rank}"
    ...
    with open(results_path, "w") as f:
        ...
        if args.phase >= 3:
            p3_study = optuna.load_study(study_name="fish_inverse_phase3", storage=storage)
