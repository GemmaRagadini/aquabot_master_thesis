"""
Tabella RMSE e MAE PER OGNI TRIAL, per il canale comando, in unita' reali
Uso:

  python3 src/net/InverseEstimator/checkpoints_inverse/metrics_per_trial_inverse.py --checkpoint src/net/InverseEstimator/checkpoints_inverse/best.pt
  python3 src/net/InverseEstimator/checkpoints_inverse/metrics_per_trial_inverse.py --checkpoint src/net/InverseEstimator/checkpoints_inverse/best.pt --all_trials
  python3 src/net/InverseEstimator/checkpoints_inverse/metrics_per_trial_inverse.py --checkpoint src/net/InverseEstimator/checkpoints_inverse/best.pt --csv_out src/net/InverseEstimator/checkpoints_inverse/metrics_inverse.csv
"""
import argparse
import os
import sys
...
# Tenuto come lista (e con i dict CH_TO_KEY / CH_UNIT) per restare speculare alla
# diretta, che itera sui canali. Aggiungere un'uscita in futuro non richiede di
# toccare la logica, solo queste costanti.
CHANNELS_1 = ["cmd"]
CH_TO_KEY  = {"cmd": "cmd"}
CH_UNIT    = {"cmd": "rad"}
...
def main():
    ...
    ap.add_argument("--scaler_path", default=os.path.join(REPO_ROOT, "src", "net", "scaler", "scalers_inverse.pkl"),
    ...
    ap.add_argument("--csv_out", default=os.path.join(SCRIPT_DIR, "metrics_per_trial_inverse.csv"))
    args = ap.parse_args()
    ...
    if input_size != ds_input:
        raise ValueError(
            ...
            f"col modello salvato (controlla N_INPUT_FEATURES nel dataset_inverse e le "
            f"feature attive in _build_windows). I risultati sarebbero privi di senso."
        )
    ...
    if out_feat != nc:
        raise ValueError(
            ...
            f"ne ha {nc}. Probabilmente N_OUTPUTS in model_inverse.py non e' allineato "
            f"ai target attivi in dataset_inverse._build_windows."
        )
    ...
    for ch in CHANNELS:
        ...
        print(f"  {ch:<12} RMSE {m['rmse_f']:.4f} ({m['rmse_fut_pct']:.1f}% ppk) | "
              f"MAE {m['mae_f']:.4f} ({m['mae_fut_pct']:.1f}% ppk) | "
              f"persist RMSE {m['rmse_persist']:.4f} MAE {m['mae_persist']:.4f} | "
